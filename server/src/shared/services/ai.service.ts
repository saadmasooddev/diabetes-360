...
import { MealDetails } from 'server/src/modules/food/repository/food.repository';
...
export interface HealthAssessmentInsightsResponse {
  insights: Array<{ name: "glucose" | "water" | "steps" | "heart_rate"; insight: string }>;
...
class AIService {
  ...
  private validateResponse<T>(
    ...
    if (typeof response.status === 'string' && response.status !== '200') {
      throw new BadRequestError(response.message || 'AI service returned an error');
    }
    ...
    if (validateData && !validateData(response.data)) {
      throw new BadRequestError(response.message || 'Invalid response data from AI service');
    }
  }
  ...
  async getNutritionalRecommendation(
    payload: Record<string, any>
      ...
      const response = await axios.post<AIBaseResponse<NutritionalRecommendation>>(
        `${this.baseUrl}/api/nutritional-recommendation/`,
        payload,
        {
          headers: {
  ...
  async getHealthAssessmentInsights(
    payload: Record<string, any>
      ...
      const response = await axios.post<AIBaseResponse<HealthAssessmentInsightsResponse>>(
        `${this.baseUrl}/api/health-assessment-insights/`,
        payload,
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      this.validateResponse(response.data, (data) => {
        return !!(data.insights && data.overall_health_summary && data.what_to_do_next);
      });
