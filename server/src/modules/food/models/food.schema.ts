...
export const dailyNutrientRecommendations = pgTable(
	"daily_nutrient_recommendations",
	{
		...
		recommendationDate: date("recommendation_date")
			.notNull()
...
export const foodScanNutrients = pgTable("food_scan_nutrients", {
	...
	proteins: numeric("proteins", { precision: 10, scale: 2 })
		.notNull()
		.default("0"),
...
export const insertDailyNutrientRecommendationSchema = createInsertSchema(
...
export const insertFoodScanNutrientsSchema = createInsertSchema(
	foodScanNutrients,
)
	.omit({
		id: true,
...
export const mealPlanMeals = pgTable("meal_plan_meals", {
	...
	mealPlanId: varchar("meal_plan_id")
		.notNull()
		.references(() => dailyMealPlans.id, { onDelete: "cascade" }),
...
export const dailyPersonalizedInsights = pgTable(
...
export const insertMealPlanMealSchema = createInsertSchema(mealPlanMeals)
	.omit({
		id: true,
		createdAt: true,
	})
...
export const insertDailyPersonalizedInsightSchema = createInsertSchema(
...
export type DailyNutrientRecommendation =
	typeof dailyNutrientRecommendations.$inferSelect;
export type InsertDailyNutrientRecommendation = z.infer<
	typeof insertDailyNutrientRecommendationSchema
>;
export type FoodScanNutrients = typeof foodScanNutrients.$inferSelect;
export type InsertFoodScanNutrients = z.infer<
	typeof insertFoodScanNutrientsSchema
>;
...
export type DailyPersonalizedInsight =
	typeof dailyPersonalizedInsights.$inferSelect;
export type InsertDailyPersonalizedInsight = z.infer<
	typeof insertDailyPersonalizedInsightSchema
>;
// Logged Meals Table - stores meals that users explicitly log after scanning (3NF normalized)
export const loggedMeals = pgTable("logged_meals", {
	id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
	userId: varchar("user_id")
		.notNull()
		.references(() => users.id, { onDelete: "cascade" }),
	mealDate: date("meal_date").notNull().default(sql`CURRENT_DATE`),
	foodName: text("food_name").notNull(), // Name of the food item scanned
	carbs: numeric("carbs", { precision: 10, scale: 2 }).notNull().default("0"),
	sugars: numeric("sugars", { precision: 10, scale: 2 }).notNull().default("0"),
	fibres: numeric("fibres", { precision: 10, scale: 2 }).notNull().default("0"),
	proteins: numeric("proteins", { precision: 10, scale: 2 })
...
export const insertLoggedMealSchema = createInsertSchema(loggedMeals)
	.omit({
		id: true,
		createdAt: true,
		updatedAt: true,
