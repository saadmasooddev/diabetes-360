...
import {
	insertFreeTierLimitsSchema,
	updateFreeTierLimitsSchema,
	insertFoodScanLimitsSchema,
	updateFoodScanLimitsSchema,
...
export class SettingsController {
	...
	async createLimits(
		...
		try {
			...
			if (body.glucoseLimit !== undefined)
				freeTierData.glucoseLimit = body.glucoseLimit;
			if (body.stepsLimit !== undefined)
				freeTierData.stepsLimit = body.stepsLimit;
			if (body.waterLimit !== undefined)
				freeTierData.waterLimit = body.waterLimit;
			if (body.discountedConsultationQuota !== undefined)
				freeTierData.discountedConsultationQuota =
					body.discountedConsultationQuota;
			if (body.freeConsultationQuota !== undefined)
				freeTierData.freeConsultationQuota = body.freeConsultationQuota;

			// Extract food scan limits
			if (body.freeUserLimit !== undefined)
				foodScanData.freeUserLimit = body.freeUserScanLimit;
			if (body.paidUserLimit !== undefined)
				foodScanData.paidUserLimit = body.paidUserScanLimit;

			// Create free tier limits if any health metric data provided
			if (Object.keys(freeTierData).length > 0) {
				const validationResult =
					insertFreeTierLimitsSchema.safeParse(freeTierData);
			...
			if (Object.keys(foodScanData).length > 0) {
				const validationResult =
					insertFoodScanLimitsSchema.safeParse(foodScanData);
				if (!validationResult.success) {
					throw validationResult.error;
				}
	...
	async updateLimits(
		...
		try {
			...
			if (Object.keys(freeTierData).length > 0) {
				const validationResult =
					updateFreeTierLimitsSchema.safeParse(freeTierData);
				if (!validationResult.success) {
					throw validationResult.error;
				}
				results.freeTierLimits =
					await this.settingsService.updateFreeTierLimits(
			...
			if (Object.keys(foodScanData).length > 0) {
				const validationResult =
					updateFoodScanLimitsSchema.safeParse(foodScanData);
				if (!validationResult.success) {
					throw validationResult.error;
				}
				results.foodScanLimits =
					await this.settingsService.updateFoodScanLimits(
						validationResult.data,
					);
			} else {
	...
	async getUserScanStatus(
		...
		try {
			...
			const isPaid = user.user.paymentType !== "free";

			const status = await this.settingsService.getUserScanStatus(
				userId,
				isPaid,
			);
