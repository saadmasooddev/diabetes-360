...
export class MedicalController {
  ...
  async createMedication(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const medication = await this.medicalService.createMedication(userId, validationResult.data);
  ...
  async getMedications(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const userId = req.user?.userId || "";
      if (!userId) {
        throw new BadRequestError("User ID not found");
      ...
      const result = await this.medicalService.getMedicationsByUserId(userId, limit, offset, skip);
  ...
  async getMedicationsByPhysicianAndDate(
    ...
    try {
      ...
      if (!physicianId || !prescriptionDate) {
        throw new BadRequestError("physicianId and prescriptionDate are required");
      }
  ...
  async uploadLabReport(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const userId = req.user?.userId || "";
      if (!userId) {
        throw new BadRequestError("User ID not found");
  ...
  async getLabReports(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const userId = req.user?.userId || "";
      if (!userId) {
        throw new BadRequestError("User ID not found");
  ...
  async updateLabReport(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const userId = req.user?.userId || "";
      if (!userId) {
        throw new BadRequestError("User ID not found");
      ...
      const report = await this.medicalService.updateLabReport(reportId, userId, req.file);
  ...
  async deleteLabReport(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const userId = req.user?.userId || "";
      if (!userId) {
        throw new BadRequestError("User ID not found");
  ...
  async downloadLabReport(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const userId = req.user?.userId || "";
      if (!userId) {
        throw new BadRequestError("User ID not found");
      ...
      const { filePath, fileName } = await this.medicalService.downloadLabReport(reportId, userId);
