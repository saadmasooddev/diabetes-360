...
import { MedicineDosage } from "../repository/medical.repository";

// Medications Table - stores prescriptions from consultations
export const medications = pgTable("medications", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: varchar("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  consultationId: varchar("consultation_id").notNull().references(() => bookedSlots.id, { onDelete: "cascade" }),
  physicianId: varchar("physician_id").notNull().references(() => users.id, { onDelete: "restrict" }),
