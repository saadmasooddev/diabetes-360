...
export class BookingRepository {
  ...
  getSlotTimeISO(
    ...
    const slotTimeIso = DateManager.slotTimeToISO(
      availabilityDate,
      slotTimeISO,
      ...
      try {
        ...
        const bookedSlotsForUserReminderPromise = tx
          .select({
            ...
            slotType: sql<string>`
						  CASE ${slotType.type}
							  WHEN ${SLOT_TYPE.ONLINE} THEN 'Online Appointment'
								WHEN ${SLOT_TYPE.ONSITE} THEN 'Onsite Appointment'
								ELSE 'Appointment'
							END
						`,
          })
        ...
        physiciansToRemind.forEach((p) => {
          reminderData.push({
            ...p,
            isPhysician: true,
            startTimeIso: this.getSlotTimeISO(
              p.availabilityDate,
              p.startTime,
              p.timeZone,
            ),
            endTimeIso: this.getSlotTimeISO(
              p.availabilityDate,
              p.startTime,
              p.timeZone,
            ),
        ...
        await tx
          ...
          .where(inArray(bookedSlots.id, bookingIdsWithNotificationsSent));
      } catch (error) {
        console.log(error);
        throw error;
      }
