...
export class BookingService {
  ...
  async bookSlot(
    ...
    const confirmCustomerNotificationPromise = this.pushNotificationService.sendDataOnlyToUser(customerId, {
    ...
    const physcianSlotBookedNotificationPromise = this.pushNotificationService.sendDataOnlyToUser(physicianId, {
      type: PUSH_MESSAGE_TYPE_ENUM.APPOINTMENT_BOOKED,
      title: "Appointment Booked",
  ...
  async getLatestPhysicianTrackingPatient(patientId: string) {
    return await this.bookingRepository.getLatestPhysicianTrackingPatient(patientId)
  }
