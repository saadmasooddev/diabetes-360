import { UserConsultationQuota } from "../models/consultation-quota.schema";
...
export class ConsultationService {
  ...
  async getUserConsultationQuota(userId: string): Promise<{discountedConsultationsUsed: number, freeConsultationsUsed: number}> {
    const quota = await this.consultationQuotaRepository.getUserConsultationQuota(userId);
    if(!quota){ 
      return {
  ...
  async getOrCreateUserConsultationQuota(userId: string): Promise<UserConsultationQuota> {
    return await this.consultationQuotaRepository.getOrCreateUserConsultationQuota(userId)
  }
}