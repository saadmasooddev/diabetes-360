...
import { medicalService, type Medication, type LabReport } from '@/services/medicalService';
import { API_ENDPOINTS } from '@/config/endpoints';

// Medications Hooks
export const useMedications = (params?: { limit?: number; offset?: number }) => {
  return useQuery<{
    ...
    queryKey: [API_ENDPOINTS.MEDICAL.MEDICATIONS, params?.limit, params?.offset],
...
export const useMedicationsByPhysicianAndDate = (
  ...
  return useQuery<{
    ...
    queryKey: [API_ENDPOINTS.MEDICAL.MEDICATIONS_BY_PHYSICIAN, physicianId, prescriptionDate],
    queryFn: () => medicalService.getMedicationsByPhysicianAndDate(physicianId!, prescriptionDate!),
...
export const useDownloadLabReport = () => {
  ...
  return useMutation({
    mutationFn: async (reportId: string) => {
      const { blob, fileName } = await medicalService.downloadLabReport(reportId);
