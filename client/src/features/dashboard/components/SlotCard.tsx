...
function hasOfflineType(slot: Slot): boolean {
  return slot.types?.some(
    (t) => t.type.toLowerCase() === 'onsite' || t.type.toLowerCase() === 'offline'
  ) || false;
}

export function SlotCard({ slot, isSelected, onClick, defaultDisplay }: SlotCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-2">
        <Clock className="h-4 w-4" style={{ color: isSelected ? '#00856F' : '#6B7280' }} />
        <div className="flex flex-col">
          ...
          <span className="text-xs text-gray-500">to {formatTime12(slot.endTime)}</span>
        </div>
      </div>
