...
export function MedicationsTable({ limit = 5, showSeeAll = true }: MedicationsTableProps) {
  const [, setLocation] = useLocation();
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [page, setPage] = useState(1);
  const pageLimit = 10;

  const { data: medicationsData, isLoading } = useMedications({ limit, offset: 0 });
  ...
  const handleRowClick = (medication: Medication) => {
    const date = new Date(medication.prescriptionDate).toISOString().split('T')[0];
    setLocation(`${ROUTES.MEDICATIONS}?physicianId=${medication.physicianId}&date=${date}`);
  };
  ...
  if (isLoading) {
    return (
      <Card className="p-4 sm:p-6" style={{ background: '#FFFFFF', border: '1px solid rgba(0, 0, 0, 0.1)', borderRadius: '12px' }}>
  ...
  if (medications.length === 0) {
    return (
      <Card className="p-6 text-center text-gray-500 text-sm sm:text-base" style={{ background: '#FFFFFF', border: '1px solid rgba(0, 0, 0, 0.1)', borderRadius: '12px' }}>
    ...
    <>
            ...
            <div className="space-y-2">
              ...
              <div>
                ...
                {medication.physician?.specialty && (
                  <p className='text-xs text-gray-700'>{medication.physician.specialty}</p>
                )}
              </div>
              <div>
                <p className="text-xs text-gray-400 mb-1">Prescription</p>
                <p className="text-xs text-gray-700">
                  {medication.medicines.length} medicine{medication.medicines.length !== 1 ? 's' : ''} prescribed
                </p>
              </div>
            </div>
          </Card>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-gray-500 text-xs sm:text-sm">Doctor</TableHead>
                <TableHead className="text-gray-500 text-xs sm:text-sm">Prescription</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {medications.map((medication) => (
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-w-[95vw] sm:max-w-4xl max-h-[90vh] overflow-y-auto mx-2 sm:mx-4">
          <DialogHeader>
            <DialogTitle className="text-lg sm:text-xl" style={{ color: '#00856F' }}>All Medications</DialogTitle>
          </DialogHeader>
          {isLoadingAll ? (
            ...
            <p className="text-center text-gray-500 py-6 sm:py-8 text-sm sm:text-base">No medications found</p>
          )}
        </DialogContent>
      </Dialog>
    </>
