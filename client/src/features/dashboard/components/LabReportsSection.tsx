...
import { useLabReports, useUploadLabReport, useUpdateLabReport, useDeleteLabReport, useDownloadLabReport } from '@/hooks/mutations/useMedical';
import { formatDate } from '@/lib/utils';
import { Skeleton } from '@/components/ui/skeleton';
import { Upload, Download, Trash2, Edit, FileText, Loader2 } from 'lucide-react';
      ...
      <>
        ...
        <Dialog open={isUploadModalOpen} onOpenChange={setIsUploadModalOpen}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle style={{ color: '#00856F' }}>Upload Lab Report</DialogTitle>
              <DialogDescription>
                Please select a PDF file to upload (max 10MB)
              </DialogDescription>
            </DialogHeader>
  ...
  return (
    <>
            ...
            {reports.map((report) => (
              ...
              >
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  >
                    <FileText className="w-5 h-5" style={{ color: '#00856F' }} />
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500">
                      {formatUploadDate(report.uploadedAt)} • {formatFileSize(report.fileSize)}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2 ml-4">
                  ...
                  >
                    {downloadMutation.isPending ? (
                      ...
                      <Download className="h-4 w-4" style={{ color: '#00856F' }} />
                    )}
                  </Button>
                  <Button
                    variant="ghost"
      ...
      <Dialog open={isUpdateModalOpen} onOpenChange={setIsUpdateModalOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle style={{ color: '#00856F' }}>Update Lab Report</DialogTitle>
            <DialogDescription>
              Replace the current file with a new PDF (max 10MB)
            </DialogDescription>
          </DialogHeader>
          {selectedReport && (
            <div className="space-y-4">
              ...
              >
                <Upload className="w-8 h-8 text-gray-400 mb-2" />
                <p className="text-sm text-gray-600">Click to select new PDF</p>
                <p className="text-xs text-gray-400 mt-1">Max 10MB</p>
              </label>
