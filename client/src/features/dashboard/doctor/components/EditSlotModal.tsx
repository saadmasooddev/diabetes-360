...
import { formatDate, convert12To24Hour, DateManager, formatTime12 } from "@/lib/utils";
  ...
  useEffect(() => {
    if (open) {
      if (isEditMode && slot) {
        ...
        setSelectedLocationId(slot.locations && slot.locations.length > 0 ? slot.locations[0].id : "");
      } else {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="p-4 max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{isEditMode ? "Edit Slot" : "Create Custom Slot"}</DialogTitle>
          <DialogDescription>
            {isEditMode
          ...
          <div className="p-4 bg-teal-50 border border-teal-200 rounded-lg">
            ...
            {isEditMode && (
              <p className="text-xs text-teal-700 mt-1">
                {isCustom ? "Custom slot - times can be edited" : "Standard slot - times cannot be edited"}
              </p>
            )}
          </div>
