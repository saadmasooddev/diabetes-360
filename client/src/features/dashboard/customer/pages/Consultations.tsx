...
import { useMyConsultations, useMarkConsultationAttended } from '@/hooks/mutations/useBooking';
...
export function Consultations() {
  ...
  const { data: upcomingData, isLoading: isLoadingUpcoming } = useMyConsultations({
    type: 'upcoming',
    page: 1,
    limit: 1,
  });

  const { data: upcomingAllData, isLoading: isLoadingUpcomingAll } = useMyConsultations({
    type: 'upcoming',
    page: upcomingPage,
    limit: pageLimit,
  });
  ...
  const markAttendedMutation = useMarkConsultationAttended();
  ...
  const handleViewDetails = (consultation: Consultation) => {
    ...
    if (!consultation.isAttended) {
      markAttendedMutation.mutate(consultation.id);
    }
  };
  ...
  const getRecommendedPhysicians = (): Physician[] => {
    ...
    const sortedByRating = physicians.sort((a, b) => (b.rating || 0) - (a.rating || 0));
  ...
  const formatConsultationTime = (dateString: string, startTime: string, endTime: string): string => {
    const date = new Date(dateString);
    ...
    <div className="flex min-h-screen" style={{ background: '#F7F9F9' }}>
      ...
      <main className="flex-1 flex justify-center items-start pt-4 sm:pt-6 lg:pt-8 pb-4 sm:pb-6 lg:pb-8 px-2 sm:px-4 lg:px-6">
        <div className="w-full max-w-[1100px]">
          ...
          <div className="mb-4 sm:mb-6">
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold mb-2" style={{ color: '#00856F' }}>
              Consultations
            </h1>
            <p className="text-xs sm:text-sm lg:text-base text-gray-600">
              Talk to our care experts and get guidance that truly understands your journey.
            </p>
          </div>
          ...
          <div className="mb-6 sm:mb-8">
            ...
            {isLoadingUpcoming ? (
              ...
              <Card className="p-3 sm:p-4 lg:p-6" style={{ background: '#FFFFFF', border: '1px solid rgba(0, 0, 0, 0.1)', borderRadius: '12px' }}>
                <div className="flex flex-col gap-3 sm:gap-4 lg:gap-6">
                  <div className="flex-1">
                    <h3 className="text-base sm:text-lg font-semibold mb-1" style={{ color: '#00856F' }}>
                      {upcomingConsultation.slot.physician.firstName} {upcomingConsultation.slot.physician.lastName}
                    </h3>
                    <p className="text-xs sm:text-sm text-gray-500 mb-3 sm:mb-4">
                      {upcomingConsultation.slot.physician.specialty || 'Physician'}
                    </p>
          ...
          <div className="mb-6 sm:mb-8">
            ...
            <div className="flex gap-3 sm:gap-4 overflow-x-auto pb-2 sm:pb-4 -mx-2 sm:mx-0 px-2 sm:px-0" style={{ scrollbarWidth: 'thin', scrollbarColor: '#E0F2F1 transparent' }}>
              {recommendedPhysicians.length > 0 ? (
                recommendedPhysicians.map((physician) => (
                  ...
                  >
                    <div className="flex flex-col items-center text-center">
                      ...
                      <h3 className="font-semibold mb-1 text-sm sm:text-base" style={{ color: '#00856F' }}>
                        {physician.firstName} {physician.lastName}
                      </h3>
                      <p className="text-xs text-gray-500 mb-1">{physician.specialty || 'Physician'}</p>
                      <p className="text-xs text-gray-500 mb-2">{physician.experience || '1+ years'}</p>
                ...
                <p className="text-sm text-gray-500 text-center w-full py-4">No recommended experts available</p>
              )}
            </div>
          </div>
          ...
          <div className="mb-6 sm:mb-8">
            <Button
              ...
              style={{ background: '#00856F', color: '#FFFFFF', height: '44px', minHeight: '44px' }}
              size="lg"
            >
      ...
      <Dialog open={isDetailsModalOpen} onOpenChange={setIsDetailsModalOpen}>
        <DialogContent className="max-w-[95vw] sm:max-w-2xl max-h-[90vh] overflow-y-auto mx-2 sm:mx-4">
          <DialogHeader>
            <DialogTitle className="text-lg sm:text-xl" style={{ color: '#00856F' }}>Consultation Details</DialogTitle>
          </DialogHeader>
          {selectedConsultation && (
            <div className="space-y-4 p-4 sm:p-6">
              <div>
                <h3 className="font-semibold mb-2 text-base sm:text-lg" style={{ color: '#00856F' }}>
                  {selectedConsultation.slot.physician.firstName} {selectedConsultation.slot.physician.lastName}
                </h3>
              ...
              <div className="space-y-3">
                <div className="flex items-start gap-2">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs sm:text-sm text-gray-700">
                      {formatConsultationDate(selectedConsultation.slot.availability.date)}
                    </p>
                  </div>
                </div>
                <div className="flex items-start gap-2">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs sm:text-sm text-gray-700">
                      {formatTime12(selectedConsultation.slot.startTime)} - {formatTime12(selectedConsultation.slot.endTime)}
                    </p>
                  </div>
                </div>
                <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="text-xs text-gray-400 mb-1">Consultation Type</p>
      ...
      <Dialog open={isUpcomingModalOpen} onOpenChange={setIsUpcomingModalOpen}>
        <DialogContent className="max-w-[95vw] sm:max-w-4xl max-h-[90vh] overflow-y-auto mx-2 sm:mx-4">
          <DialogHeader>
            <DialogTitle className="text-lg sm:text-xl" style={{ color: '#00856F' }}>All Upcoming Consultations</DialogTitle>
          </DialogHeader>
          {isLoadingUpcomingAll ? (
            ...
            <p className="text-center text-gray-500 py-6 sm:py-8 text-sm sm:text-base">No upcoming consultations</p>
          )}
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isPastModalOpen} onOpenChange={setIsPastModalOpen}>
        <DialogContent className="max-w-[95vw] sm:max-w-4xl max-h-[90vh] overflow-y-auto mx-2 sm:mx-4">
          <DialogHeader>
            <DialogTitle className="text-lg sm:text-xl" style={{ color: '#00856F' }}>All Past Consultations</DialogTitle>
          </DialogHeader>
          {isLoadingPastAll ? (
            <div className="space-y-3 sm:space-y-4 p-4 sm:p-6">
              {[1, 2, 3].map((i) => (
            ...
            <p className="text-center text-gray-500 py-6 sm:py-8 text-sm sm:text-base">No past consultations</p>
          )}
        </DialogContent>
      </Dialog>
    </div>
