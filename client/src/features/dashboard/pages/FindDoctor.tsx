...
function mapPhysicianToDoctor(physician: Physician) {
  return {
    ...
    consultationFee: physician.consultationFee ? parseFloat(physician.consultationFee) : undefined,
  };
}
...
export function FindDoctor() {
  ...
  const [selectedSlotTypeId, setSelectedSlotTypeId] = useState<string | null>(null);
  ...
  const searchValue = debouncedSearchQuery.trim() === '' || debouncedSearchQuery.toLowerCase() === 'all'
    ? undefined
  ...
  useEffect(() => {
    if (physiciansError) {
      toast({
        title: 'Error',
        description: physiciansError instanceof Error ? physiciansError.message : 'Failed to load physicians',
        variant: 'destructive',
      });
    }
  ...
  const { data: bookingPrice, isLoading: isLoadingPrice } = useCalculateBookingPrice(selectedPhysician?.id || null);
  ...
  const handleDateSelect = (date: Date | undefined) => {
    ...
    if (selected.getMonth() !== calendarMonth.getMonth() ||
          ...
          <div className="w-full px-4 sm:px-6 lg:px-8 flex justify-center">
            {selectedPhysician && selectedDate && selectedSlot && (
              <ConfirmationScreen
                ...
                hospital={{ id: '1', name: selectedSlot.types?.map((t: any) => t.type).join(', ') || 'Online' }}
