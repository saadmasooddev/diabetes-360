...
export function UserHealthTargets() {
	...
	const recommendedGlucose = targets?.recommended.find(
	...
	useEffect(() => {
		if (targets) {
			const userGlucose = targets.user.find(
				(t) => t.metricType === METRIC_TYPE_ENUM.BLOOD_GLUCOSE,
			);
			...
			setGlucoseTarget(
				userGlucose
					? parseFloat(userGlucose.targetValue).toString()
					: recommendedGlucose
						? parseFloat(recommendedGlucose.targetValue).toString()
						: "",
			);
			setStepsTarget(
				userSteps
					? parseFloat(userSteps.targetValue).toString()
					: recommendedSteps
	...
	const handleSaveAll = async () => {
		...
		upsertMutation.mutate(targetsToSave);
	};
		...
		>
			<CardHeader className="p-4 sm:p-6">
				...
				<CardDescription className="text-sm">
					Set your personal target values for health metrics. If not set,
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
							...
							<div className="space-y-2">
								...
								<div className="flex gap-2">
									...
									{hasUserTarget(METRIC_TYPE_ENUM.STEPS) && (
										<Button
											onClick={() => handleDelete(METRIC_TYPE_ENUM.STEPS)}
											disabled={isSubmitting}
											variant="outline"
											size="icon"
										>
							...
							{isPaidUser && (
								<div className="space-y-2">
									<Label
										htmlFor="heart-rate-target"
										className="text-sm font-medium text-gray-700"
									>
										Heart Rate Target (bpm)
										{hasUserTarget(METRIC_TYPE_ENUM.HEART_RATE) && (
									...
									<div className="flex gap-2">
										...
										{hasUserTarget(METRIC_TYPE_ENUM.HEART_RATE) && (
											<Button
												onClick={() =>
													handleDelete(METRIC_TYPE_ENUM.HEART_RATE)
												}
												disabled={isSubmitting}
												variant="outline"
												size="icon"
									...
									<p className="text-xs text-gray-500">
										{recommendedHeartRate &&
											!hasUserTarget(METRIC_TYPE_ENUM.HEART_RATE) &&
											`Recommended: ${parseFloat(recommendedHeartRate.targetValue)} bpm`}
