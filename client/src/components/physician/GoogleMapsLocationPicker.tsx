			...
			(
				...
				if (status === "OK" && results && results[0]) {
					...
					const getComponent = (type: string) => {
						const component = addressComponents.find((ac) =>
		...
		(prediction: google.maps.places.PlacePrediction | string) => {
			...
			place
				...
				.then(() => {
					...
					if (place.location) {
						const getComponent = (type: string) => {
							const component = place.addressComponents?.find((ac) =>
								ac.types.includes(type),
							);
	...
	return {
		...
		resetSession: () => {
			sessionTokenRef.current =
				null as google.maps.places.AutocompleteSessionToken | null;
			setSuggestions([]);
		},
		placesLib,
	};
