...
import { useSlotSizes, useSlotTypes, useDatesWithAvailability, useSlotsForDate, useCreateSlots, useDeleteSlot, useUpdateSlotLocations } from '@/hooks/mutations/useBooking';
...
export function PhysicianAvailabilityManagement() {
  ...
  const { data: datesWithAvailability = [], isLoading: isLoadingDates } = useDatesWithAvailability(user?.id || null);
                  ...
                  {isLoadingSlots ? (
                    ...
                    slots.map((slot) => (
                      <SlotCard
                        ...
                        deleteSlotMutation={deleteSlotMutation}
...
function CreateSlotsForm({ slotSizes, slotTypes, onSubmit, onCancel, isLoading }: CreateSlotsFormProps) {
  ...
  const { data: locations = [], isLoading: isLoadingLocations } = usePhysicianLocations();
    ...
    <form onSubmit={handleSubmit} className="space-y-4 p-2 sm:p-4">
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-gray-500">
          Consultation fees are set in your profile settings and will be used for all bookings.
        </p>
      </div>
      ...
      {hasOfflineType && (
        <div className="space-y-2">
          ...
          {hasOfflineType && !selectedLocationId && activeLocations.length > 0 && (
            <p className="text-sm text-red-600">Please select a location for offline consultations</p>
          )}
        </div>
      )}
...
interface SlotCardProps {
  ...
  deleteSlotMutation: ReturnType<typeof useDeleteSlot>;
              ...
              {activeLocations.length === 0 ? (
                ...
                <RadioGroup value={editingLocationId || ''} onValueChange={(value) => onLocationIdChange(value || null)}>
                  <div className="space-y-2 max-h-40 overflow-y-auto border rounded-md p-2">
                    {activeLocations.map((location) => (
                      <div key={location.id} className="flex items-start space-x-2">
                        <RadioGroupItem value={location.id} id={`edit-loc-${location.id}`} className="mt-1" />
