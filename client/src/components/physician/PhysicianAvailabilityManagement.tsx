...
import { DateManager, cn, formatTime12 } from "@/lib/utils";
	...
	const handleCreateSlots = async (data: {
		...
		try {
			...
			queryClient.invalidateQueries({
				queryKey: ["booking", "dates-with-availability", user?.id],
			});
...
function CreateSlotsForm({
	...
	const totalSlots =
		selectedSlotSize && startTime && endTime
	...
	const getValidEndTimes = () => {
		...
		for (
			let i = selectedSlotSize.size;
			i <= 24 * 60;
			i += selectedSlotSize.size
		) {
		...
		<form onSubmit={handleSubmit} className="space-y-4 p-2 sm:p-4">
			...
			<div className="space-y-2">
				...
				<p className="text-xs text-gray-500">
					Consultation fees are set in your profile settings and will be used
					for all bookings.
				</p>
			</div>
					...
					{isLoadingLocations ? (
						...
						<div className="p-3 bg-yellow-50 border border-yellow-200 rounded-md">
							<p className="text-sm text-yellow-800">
								No active locations found. Please add locations in the Manage
								Locations section first.
							</p>
						</div>
					) : (
						<RadioGroup
					...
					{hasOfflineType &&
						...
						activeLocations.length > 0 && (
							<p className="text-sm text-red-600">
								Please select a location for offline consultations
							</p>
						)}
				</div>
			)}
