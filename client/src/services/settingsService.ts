...
export type ExtendedLimits = FreeTierLimits & {
...
class SettingsService {
	...
	async getLimits(): Promise<
		FreeTierLimits & { foodScanLimits?: { freeTier: number; paidTier: number } }
	> {
		const response = await httpClient.get<ApiResponse<ExtendedLimits>>(
	...
	> {
		const response = await httpClient.post<
			...
			>
		>(API_ENDPOINTS.SETTINGS.LIMITS, data);
		if (!response.success || !response.data) {
	...
	async getFoodScanStatus(): Promise<FoodScanStatus> {
		const response = await httpClient.get<ApiResponse<FoodScanStatus>>(
			API_ENDPOINTS.SETTINGS.FOOD_SCAN_STATUS,
		);
		if (!response.success || !response.data) {
