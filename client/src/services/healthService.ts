...
class HealthService {
	async getLatestMetric(): Promise<LatestMetrics> {
		const response = await httpClient.get<ApiResponse<LatestMetrics>>(
			`${API_ENDPOINTS.HEALTH.LATEST}?date=${new Date().toISOString()}`,
		);
		if (!response.success) {
	...
	async getAggregatedStatistics(): Promise<AggregatedStatistics> {
		const response = await httpClient.get<ApiResponse<AggregatedStatistics>>(
			`${API_ENDPOINTS.HEALTH.STATISTICS}?date=${new Date().toISOString()}`,
		);
		if (!response.success || !response.data) {
			throw new Error(
				response.message || "Failed to fetch aggregated statistics",
			);
		}
	...
	async getRecommendedTargets(): Promise<
		Array<{
			id: string;
			userId: string | null;
			metricType: MetricType;
	...
	> {
		...
		if (!response.success || !response.data) {
			throw new Error(
				response.message || "Failed to fetch recommended targets",
			);
		}
		return response.data;
	}
	...
	> {
		const response = await httpClient.post<
			...
			>
		>(API_ENDPOINTS.HEALTH.TARGETS.USER_BATCH, { targets });
		if (!response.success || !response.data) {
	...
	async getHealthInsights(): Promise<HealthInsights> {
		const response = await httpClient.get<ApiResponse<HealthInsights>>(
			`${API_ENDPOINTS.HEALTH.INSIGHTS}?date=${new Date().toISOString()}`,
		);
		if (!response.success || !response.data) {
	...
	> {
		const response = await httpClient.post<
			...
			>
		>(API_ENDPOINTS.HEALTH.TARGETS.RECOMMENDED_BATCH, { targets });
		if (!response.success || !response.data) {
			throw new Error(
				response.message || "Failed to update recommended targets",
			);
		}
		return response.data;
	}
	...
	async uploadGlucoseMeterImage(
		...
		const response = await httpClient.post<
			ApiResponse<{ blood_sugar_reading: string }>
		>(API_ENDPOINTS.HEALTH.UPLOAD_GLUCOSE_IMAGE, formData, {
			headers: {
		...
		if (!response.success || !response.data) {
			throw new Error(
				response.message || "Failed to upload glucose meter image",
			);
		}
		return response.data;
	}
