...
class AdminService {
	async getAllUsers(): Promise<User[]> {
		const response = await httpClient.get<ApiResponse<{ users: User[] }>>(
	...
	async getUserById(id: string): Promise<{ user: User; customerData?: any }> {
		const response = await httpClient.get<
			ApiResponse<{ user: User; customerData?: any }>
	...
	async createUser(data: CreateUserRequest): Promise<User> {
		const response = await httpClient.post<ApiResponse<{ user: User }>>(
			API_ENDPOINTS.ADMIN.USERS,
			data,
		);
		if (!response.success || !response.data) {
	...
	async updateUser(id: string, data: UpdateUserRequest): Promise<User> {
		const response = await httpClient.put<ApiResponse<{ user: User }>>(
			`${API_ENDPOINTS.ADMIN.USERS}/${id}`,
			data,
		);
		if (!response.success || !response.data) {
	...
	async toggleUserStatus(id: string, isActive: boolean): Promise<User> {
		const response = await httpClient.patch<ApiResponse<{ user: User }>>(
