...
class BookingService {
	...
	async getAvailabilityDates(): Promise<AvailabilityDate[]> {
		const response = await httpClient.get<
			ApiResponse<{ dates: AvailabilityDate[] }>
	...
	async createAvailabilityDate(date: string): Promise<AvailabilityDate> {
		const response = await httpClient.post<
			ApiResponse<{ availability: AvailabilityDate }>
		>(API_ENDPOINTS.BOOKING.AVAILABILITY_DATES, { date });
		if (!response.success || !response.data) {
	...
	async getDatesWithAvailability(physicianId: string): Promise<string[]> {
		...
		if (!response.success || !response.data) {
			throw new Error(
				response.message || "Failed to fetch dates with availability",
			);
		}
		return response.data.dates;
	}
	...
	async getSlotsForDate(physicianId: string, date: string): Promise<Slot[]> {
		const response = await httpClient.get<ApiResponse<{ slots: Slot[] }>>(
			`${API_ENDPOINTS.BOOKING.PHYSICIAN_SLOTS(physicianId)}?date=${DateManager.formatDate(date)}`,
		);
		if (!response.success || !response.data) {
	...
	async getPhysicianDatesWithSlots(params: {
		...
		if (!response.success || !response.data) {
			throw new Error(
				response.message || "Failed to fetch physician dates and slots",
			);
		}
	...
	async calculateBookingPrice(
		...
		const response = await httpClient.get<
			ApiResponse<{ price: BookingPriceCalculation }>
	...
	async updateSlotLocations(
		slotId: string,
		data: UpdateSlotLocationsRequest,
	): Promise<PhysicianLocation[]> {
		const response = await httpClient.patch<
			ApiResponse<{ locations: PhysicianLocation[] }>
		>(API_ENDPOINTS.BOOKING.UPDATE_SLOT_LOCATIONS(slotId), data);
		if (!response.success || !response.data) {
	...
	async getDatesWithBookings(
		month: number,
		year: number,
	): Promise<DatesWithSpecifiedDateSlots> {
		const queryParams = new URLSearchParams();
		queryParams.append("month", month.toString());
		queryParams.append("year", year.toString());

		const response = await httpClient.get<
			ApiResponse<DatesWithSpecifiedDateSlots>
		>(`${API_ENDPOINTS.BOOKING.DATES_WITH_BOOKINGS}?${queryParams.toString()}`);

		if (!response.success || !response.data) {
			throw new Error(
				response.message || "Failed to fetch dates with bookings",
			);
		}
		return response.data;
	}
	...
	async createCustomSlot(data: {
		...
		const response = await httpClient.post<ApiResponse<{ slot: Slot }>>(
			API_ENDPOINTS.BOOKING.CREATE_CUSTOM_SLOT,
			data,
		);

		if (!response.success || !response.data) {
			throw new Error(response.message || "Failed to create custom slot");
		}
